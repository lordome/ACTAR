...
void Run()
{
	...
	if (Run == 0)
		digname = "./root_files/dig_files/digFile_alpha7MeV_iC4H10_204mb_em3.root";
	// if(Run==1)digname="./root_files/dig_files/digFile_alpha7MeV_iC4H10_144mb_em3.root";
	if (Run == 1)
		digname = "./root_files/dig_files/digFile_alpha7MeV_iC4H10_144mbar_z1_em3.root";
	...
	if (Run == 2)
		digname = "./root_files/dig_files/digFile_gasBox105x85x105_alpha_5.8MeV_VA0_piotrdigit.root";
	...
	if (Run == 2)
		Double_t driftVelocity = 36.43E-3; // p=105mbar
	...
	if (Run == 0)
		const Int_t ActarBin = 111;
	if (Run == 1)
		const Int_t ActarBin = 153;
	// if(Run==2)const Int_t ActarBin=213;
	if (Run == 2)
		const Int_t ActarBin = 250;

	if (Run == 2)
		const Int_t ActarLatBin = 150;
	...
	if (Run == 0)
		simname = "/home/piotr/workplace/analysis_macros/Profiles/alphaLongProfile/alphadEvsR_7MeV_iC4H10_204mbar_ActarSim_VAngle0_HAngle0_em3.txt"; // L113
	if (Run == 1)
		simname = "/home/piotr/workplace/analysis_macros/Profiles/alphaLongProfile/alphadEvsR_7MeV_iC4H10_144mbar_ActarSim_VAngle0_HAngle0_em3.txt"; // L153
	...
	if (Run == 2)
		simname = "/home/piotr/workplace/analysis_macros/Profiles/NewProjAlphaLongProfile/alphadEvsR_3alpha_iC4H10_105mbar_ActarSim_VA0_thr3_4_z100_D19.txt"; // L250
	...
	if (myfile.is_open())
	{
		for (int k = 0; k < ActarBin; k++)
		{
			...
			if (dE_elec > ActarSimdEMax)
			{
				ActarSimdEMax = dE_elec;
				ActarSimdEMaxPos = Range;
				ActarSimdEMaxPosBin = k;
			}
	...
	if (Run == 2)
		simlatname = "/home/piotr/workplace/analysis_macros/Profiles/LatProf/alphaLatRvsR_run974_VA-10_alpha_5.8MeV_PadBin_NoTeeth.txt"; // L150
		...
		if (numberofpads > 0)
		{
			...
			if (!simuFlag || (simuFlag && jentry % beamOn == (beamOn - 1))) // To have the 2 parts of the reaction in simu
			{
							...
							if (simuFlag)
							{
								...
								*/
								...
								if (c >= 23)
								{
									...
									if (c > 23 && c < numberOfColumns - 1 && padCharge[r][c - 1] <= threshold && padCharge[r][c + 1] <= thresholdADC)
									{
										NTeeth[r]++;
									} // padCharge[r][c]=0;}// cout<<"TEETH : C "<<c<<" R "<<r<<" padCharge[r][c] "<<padCharge[r][c]<<endl; }
									if (c < numberOfColumns - 2 && padCharge[r][c + 1] <= thresholdADC && padCharge[r][c + 2] > thresholdADC)
										NRHoles[r]++;
								}
				...
				{ // LOOP on Rows
					...
					for (Int_t c = 23; c < numberOfColumns; c++)
					{ // LOOP on Columns
						if (padCharge[r][c] > threshold && NBlock == 0 && padCharge[r][c + 1] <= threshold)
							NBlock++;
						else if (padCharge[r][c] > threshold && NBlock == 1)
						{
							MultiBlock = 1;
						} // cout<<"r "<<r<<" c "<<c<<" MultiBlock "<<endl;}
					}
				}
				...
				if (TrackAnalysis && Candidate)
				{
					...
					IniPoint[0] = 0;
					IniPoint[1] = a * IniPoint[0] + b / 2;
					FinPoint[0] = numberOfColumns;
					FinPoint[1] = FinPoint[0] * a + b / 2;
					...
					if (AlphaBraggFitFlag)
					{
						...
						for (int k = 0; k < ActarBin; k++)
						{
							...
							if (startbin == 0 && ActarSimrange[k] - RShift >= 0)
								startbin = k;
							else if (startbin != 0 && endbin == 0 && ActarSimdE[k] < 0.1)
							{
								endbin = k;
								simrange = ActarSimrange[k] - RShift;
							}

							if (ActarSimdE[k] > valmax)
							{
								valmax = ActarSimdE[k];
								maxbin = k;
								simmaxrange = ActarSimrange[k] - RShift;
							}
						...
						if ((latNChi - 2) != 0)
							RealLatchisq = RealLatchisq / (latNChi - 2);
...
void TrackPadProjection(Double_t **padCharge, Double_t **padHeight, Double_t *Start, Double_t *Stop, Int_t Rmin, Int_t Rmax, Int_t Cmin, Int_t Cmax, Double_t threshold, TH1F *h_bragg)
{
	...
	for (Int_t r = Rmin; r < Rmax + 1; r++)
	{ // Rows
		for (Int_t c = Cmin; c < Cmax + 1; c++)
		{ // columns
...
void WeightedPadProjection(Double_t **padCharge, Double_t **padHeight, Double_t *Start, Double_t *Stop, Int_t Rmin, Int_t Rmax, Int_t Cmin, Int_t Cmax, Double_t threshold, Double_t *gdEdx, Double_t *gLatRange, Double_t *gRange)
{
	...
	{ // Rows
		...
		{ // columns
			...
			if (padCharge[r][c] > threshold)
			{
				...
				if (NSlice == 1 && (Int_t)(rA / binSize) >= 0 && (Int_t)(rA / binSize) < NBinsGlobal)
				{
					gdEdx[(Int_t)(rA / binSize)] += padCharge[r][c] / binSize;
					dQtotproj += padCharge[r][c];
					dQpad += padCharge[r][c];
				}
				else
				{
					...
					for (Int_t s = (int)(rA / binSize); s < (int)(rB / binSize); s++)
					{
						...
						if (Vertex[VN][0] > (xPad + PadSize / 2))
						{
							...
							if (Vertex[VN][1] < (yPad - PadSize / 2) || Vertex[VN][1] > (yPad + PadSize / 2))
								cout << "Problem with pad projection, close side, Col " << c << " Row " << r << endl;
						}
						...
						if (Vertex[VN][0] < (xPad - PadSize / 2))
						{
							...
							tmpVertex[VN][1] = Vertex[VN][1] = PointI[1] + (PointI[0] - Vertex[VN][0]) * (Stop[0] - Start[0]) / (Stop[1] - Start[1]);
							if (Vertex[VN][1] < (yPad - PadSize / 2) || Vertex[VN][1] > (yPad + PadSize / 2))
								cout << "Problem with pad projection, further side, Col " << c << " Row " << r << endl;
						}
	...
	if (TimerFlag)
		printf("Time at the end of projection = %f seconds\n", timer2.CpuTime());
}
...
void WeightedPadProjection3(Double_t **padCharge, Double_t **padHeight, Double_t *Start, Double_t *Stop, Int_t Rmin, Int_t Rmax, Int_t Cmin, Int_t Cmax, Double_t threshold, Bool_t PadBin, Double_t *gdEdx, Double_t *gLatRange, Double_t *gRange)
{
	...
	float binSize;
	if (PadBin)
		binSize = PadSize / TMath::Cos(VAngle * rad) / TMath::Cos(HAngle * rad);
	else
		binSize = 300 / (float)NBinsGlobal;
	...
	{ // Rows
		...
		{ // columns
			...
			if (padCharge[r][c] > threshold)
			{
				...
				if ((rB / binSize - (float)(int)(rB / binSize)) == 0)
				{
					NSlice--;
					cout << "Projection of pad corner on a bin!!!" << endl;
				}
	...
	if (TimerFlag)
		printf("Time at the end of projection3 = %f seconds\n", timer2.CpuTime());
...
void chi2(Int_t &npar, Double_t *gin, Double_t &result, Double_t *par, Int_t flg)
{
	...
	for (Int_t ibin = 0; ibin < NBinsGlobal; ibin++)
	{
		...
		if (chisq < 0)
			cout << "ACHTUNG " << ibin << " nc1[ibin] " << nc1[ibin] << "   " << fac << "   " << fac2 << "  "
				 << "  " << err_nc << "   " << (fac * fac + fac2 * fac2) * err_nc << endl;
	}
