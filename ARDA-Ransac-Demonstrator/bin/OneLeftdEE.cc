...
bool InOscar( int gid ){
  ...
  if(  ( gid >  21000 )  && ( gid <  21234 ) && ( gid != 21011 )
    ...
    && ( gid != 21122 ) && ( gid != 21145 ) && ( gid != 21156 ) && ( gid != 21231 )/*
    && ( gid != 21211 ) && ( gid != 21222 ) */) {
      return true;
  }else{
      return false;
...
TVector3 VecFascio(1,0,0);

int OneLeftdEE (TString ifname="") {
 //Setting Hyperparameters
    ...
    //cout << "Set event num:" ;
    //cin >> toanalyse;
    double threshold = 1500; // minimum energy requested for a single cluster.
    double fenergy = 40000; //energy threshold for a track in order to be considered a beam track.
    double width  = 4.5; // maximum distance from model accepted in clustering
    ...
    double trsize = 12; //min number of pads required in order to consider a cluster a real track
...
// end Setting Hyperparameters
  ...
  while ( rdr.Next() ) {
    ...
    if( rdr.() == 37541 ) continue;

    energie.clear();

    if (event->getEventNumber()== (1610+277)/* toanalyse*/)     {
    ...
    for (unsigned int i=0; i<hits.size(); i++) {unfittedPoints.push_back(i);} //it's a vector containing indexes. The ones that will be included in a cluster will be ==-1.
  ...
  while (1){      //There are particular conditions, the points chosen randomly must have same y & z, the energy threshold is very high, the number of pads required has to be high.
    ...
    while (iterations<loops){
      ...
      if (alsoinliers.size()>besize && GetErrorMean<besterr && totalenergy>fenergy ) { // comparing the cluster to the previous best one.
  ...
  cFittedEvent<cPhysicalHit> tracker_withHits;
  ...
  /*if( besttracks.size() == trackcount && trackcount > 1 ){
    histogid->Fill( AA.x() );
  //  cout << "vertice.x" << AA.x() << endl;
  }
}
  ...
  /*for (auto &a : besttracks) {
       if (vectracks[a].isFittable()==false) {continue;  vectracks[a].setGone(true);}
  ...
  if( besttracks.size() == trackcount && besttracks.size() > 1 ){
    ...
    for( auto& a : besttracks ){
      for( auto& i: vectracks[a].getPoints()){
        if( hits[i].getY()>60 || ( hits[i].getX()<64 && hits[i].getX()>32 && hits[i].getY()>54 ) ){
          if(left){
            multipleleft = true;
          }
          left = true;
    ...
    if(  left && !multipleleft && AA.x() > 0 ){ //besttracks.size() == trackcount &&
      ...
      dx = sqrt( pow( hits[max].getY()-hits[min].getY(), 2 ) + pow( hits[max].getX()-hits[min].getX(), 2 ) + pow( hits[max].getZ()-hits[min].getZ(), 2 ));
      ...
      for( auto& i : hits ){
        ...
        if( InOscar( i.getGlobalChannelId() ) && i.getEnergy() > enmax ){
          enmax = i.getEnergy();
          gidmax =  i.getGlobalChannelId();
        }

      }
      if ( gidmax == 21231 ){
        //cout << "Found at 21231: " << event->getEventNumber() << endl;
        continue;
      }
      histogid->Fill( gidmax );
      histodEE->Fill( enmax , encount/dx );
      ofile << enmax  << "   "  << encount/dx << endl;
      if ( encount/dx < 100)       cout << "encount<100 at : " << event->getEventNumber()  << " en = " << encount << endl;

    }

  }*/  //ONE LEFT CHECKS

  vector<bool> vec_bool_mediax;
  vector<double> vec_mediax;
  int numbool;


  double mediax = 0;
  cout << "mediaX:  " << mediax << "  AA.x() " << AA.x() << endl;

  if ( AA.x() < 85 && AA.x() > 50 && (besttracks.size() == 3 || besttracks.size() == 4 ) ){
    for( auto& a : besttracks ){
      for( auto& i: vectracks[a].getPoints()){
        mediax+=hits[i].getX();
        }
      mediax/=vectracks[a].getPoints().size();
      cout << "mediaX:  " << mediax << "  AA.x() " << AA.x() << endl;
      if( mediax > AA.x() ) numbool++;
      mediax = 0;
      }
  } 

  cout << "numbool" << numbool << endl;
