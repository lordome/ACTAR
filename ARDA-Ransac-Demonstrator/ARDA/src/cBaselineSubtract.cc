...
class cBaselineSubctractFactory : public cSignalPreprocessorManager::abstractFactory {
  public:
    cBaselineSubctractFactory() : abstractFactory("baselineSubtract") {}
    ~cBaselineSubctractFactory() {}

    cSignalPreprocessor* createPreprocessor() {return new cBaselineSubctract;}
};

static cBaselineSubctractFactory FPNfac;

void cBaselineSubctract::preprocess(std::map<Int_t, cRawSignal> *sigs) {
  // Main loop
  for (auto &signal: *sigs) {
    ...
    // Calculate the standard deviation on the points **below** the maximum,
