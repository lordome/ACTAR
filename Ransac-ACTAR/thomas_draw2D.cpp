...
Double_t GetError(arrayV2 model, arrayD4 p, double zScale); // SQUARE! of the distance between the point and the line
...
int fit(string input_file = "input_parameters.txt")
{
    ...
    zRescalingFit = parMap["zRescalingFit"];
    ...
    vector<TH2F *> vHisto2DXY;
    vector<TH2F *> vHisto2DXZ;
    vector<TH2F *> vHisto2DYZ;

    vHisto2DXY.emplace_back(new TH2F("hist2DXYfirst", "", 128, 0, 128, 128, 0, 128));
    vHisto2DXZ.emplace_back(new TH2F("hist2DXYfirst", "", 128, 0, 128, 128, 0, 128));
    vHisto2DYZ.emplace_back(new TH2F("hist2DXYfirst", "", 128, 0, 128, 128, 0, 128));
    ...
    while (rdr.Next())
    {
        ...
        if ((!oneEventOnly || event->getEventNumber() == toAnalyse) && event->getEventNumber() > startFrom)
        {
            ...
            { // There are particular conditions, the points chosen randomly must have same y & z, the energy threshold is very high, the number of pads required has to be high.
                ...
                while (iterations < loops)
                {
                    ...
                    for (unsigned int i = 0; i < unfittedPoints.size(); i++)
                    {
                        ...
                        if (GetError(maybemodel, samplex, zRescaling) < pow(fwidth, 2) && hits[i].isTrackable())
                        {
                            alsoinliers.push_back(i);
                            sumvalue += GetError(maybemodel, samplex, zRescaling) * hits[i].getEnergy();
                            totalenergy += hits[i].getEnergy();
                        } // end if (GetError(maybemodel,samplex, zRescaling)<pow(fwidth,2) && hits[i].isTrackable())
            ...
            { // looking for normal tracks. The method used is exactly the previous one.
                ...
                while (iterations < loops)
                {
                    ...
                    for (unsigned int i = 0; i < unfittedPoints.size(); i++)
                    {
                        ...
                        if (GetError(maybemodel, samplex, zRescaling) < pow(width, 2) /*&& ( hits[i].isTrackable()*/)
                        {
                            alsoinliers.push_back(i);
                            arrayD4 samplex = {hits[i].getX(), hits[i].getY(), hits[i].getZ(), hits[i].getEnergy()};
                            sumvalue += GetError(maybemodel, samplex, zRescaling) * hits[i].getEnergy();
                ...
                if (media < 56)
                {
                    for (unsigned int i = 0; i < bestinliers.size(); i++)
                    {
                        if (hits[bestinliers[i]].getY() < 64)
                            temporary.push_back(bestinliers[i]);
                    }
                    bestinliers = temporary;
                }

                if (bestinliers.size() < trsize)
                {
                    break;
                }
            ...
            for (unsigned int i = 0; i < vectracks.size(); i++)
            {
                ...
                // cout << "BEAM?" << (vectracks[i].isFittable() ? "NO  " : "YES  ") << "Size: " << vectracks[i].getPoints().size() << " Energy: " << energy_track << " Average : " << energie[i] << endl;
            ...
            for (unsigned int a = 0; a < vectracks.size(); a++)
            {
                ...
                for (unsigned int i = 0; i < vectracks.size(); i++)
                {
                    ...
                    for (auto &h : vectracks[i].getPoints())
                    {
                        ...
                        if (GetError(model, point, zRescaling) < SqVertex_Width)
                        {
                            // cout << "trovato: "<< i << endl;
            ...
            delete hXZ;
            delete hYZ;
            hXY = new TH2F("Hist2DXY", "", 128, 0, 128, 128, 0, 128);
            hXZ = new TH2F("Hist2DXZ", "", 128, 0, 128, 100, 0, 500);
            hYZ = new TH2F("Hist2DYZ", "", 128, 0, 128, 100, 0, 500);

            for (unsigned int i = 0; i < hits.size(); i++)
            {
                for (int j = 0; j < hits[i].getEnergy() && j < 1; j++)
                {
                    hXY->Fill(hits[i].getX(), hits[i].getY());
                    hXZ->Fill(hits[i].getX(), hits[i].getZ());
                    hYZ->Fill(hits[i].getY(), hits[i].getZ());
                }
            }
            ...
            /*
              ...
              h3d->Draw("LEGO");
            */
            for (auto p : vHisto2DXY)
            {
                delete p;
            }
            for (auto p : vHisto2DYZ)
            {
                delete p;
            }
            for (auto p : vHisto2DXZ)
            {
                delete p;
            }
            ...
            TCanvas *canColor = new TCanvas("canColor", "canColor", 800, 600);
            canColor->Divide(3);

            string titleXY, titleXZ, titleYZ;

            for (unsigned int i = 0; i < vectracks.size(); ++i)
            {
                titleXY = "vHisto2DXY" + to_string(i);
                titleXZ = "vHisto2DXZ" + to_string(i);
                titleYZ = "vHisto2DYZ" + to_string(i);

                vHisto2DXY.emplace_back(new TH2F(titleXY.c_str(), titleXY.c_str(), 128, 0, 128, 128, 0, 128));
                vHisto2DXZ.emplace_back(new TH2F(titleXZ.c_str(), titleXZ.c_str(), 128, 0, 128, 75, 0, 500));
                vHisto2DYZ.emplace_back(new TH2F(titleYZ.c_str(), titleYZ.c_str(), 128, 0, 128, 75, 0, 500));
                // vector<int> index1(vectracks[i].getPoints().begin(), vectracks[i].getPoints().end());
                for (auto &it_hits : vectracks[i].getPoints())
                {
                    vHisto2DXY[i]->Fill(hits[it_hits].getX(), hits[it_hits].getY());
                    vHisto2DXZ[i]->Fill(hits[it_hits].getX(), hits[it_hits].getZ());
                    vHisto2DYZ[i]->Fill(hits[it_hits].getY(), hits[it_hits].getZ());
                }

                canColor->cd(1);
                vHisto2DXY[i]->SetFillColor(i + 1);
                vHisto2DXY[i]->Draw("BOX SAME");

                canColor->cd(2);
                vHisto2DXZ[i]->SetFillColor(i + 1);
                vHisto2DXZ[i]->Draw("BOX SAME");

                canColor->cd(3);
                vHisto2DYZ[i]->SetFillColor(i + 1);
                vHisto2DYZ[i]->Draw("BOX SAME");
            }
            ...
            for (auto &it_track : besttracks)
            {
                cout << "it_track" << it_track << endl;
                for (auto &it_hits : vectracks[it_track].getPoints())
                {
                    totalEvent->Fill(hits[it_hits].getX(), hits[it_hits].getY(), hits[it_hits].getZ());
                }
            }
...
Double_t GetError(arrayV2 model, arrayD4 p, double zScale)
{
...
extern void FunctionForTrack(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
    ...
    for (unsigned int i = 0; i < indextofit.size(); i++)
    {
        ...
        chisq += GetError(minuitmodel, pointstofit, zRescalingFit);
    }
    f = chisq;
}
...
void FunctionForVertex(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
    ...
    for (auto &i : eTrack.getLines())
    {
        ...
        err = GetError(model, p, zRescaling); // si può pesare questo test con il chiquadro di ogni traccia: più una traccia è precisa, minore sarà il suo contributo all'errore finale. /GlobalTrack.GetChi2(i);
        chisq += err;
    }
