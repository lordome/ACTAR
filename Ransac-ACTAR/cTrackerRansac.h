...
public:
    ...
    array<TVector3, 2> GetParamWithSample(array<Double_t, 4> sample1, array<Double_t, 4> sample2);
    // SQUARE! of the distance between the point and the line
    Double_t GetError_noscale(array<TVector3, 2> model, array<Double_t, 4> p);
    // SQUARE! of the distance between the point and the line
    Double_t GetError(std::array<TVector3, 2> model, array<Double_t, 4>  p, double zScale);
    // function used to test the clusters.
    Double_t ClusterTest(double &sumvalue, double &totalenergy, vector<T> &inliers);
