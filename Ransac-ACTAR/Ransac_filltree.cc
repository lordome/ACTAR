...
/// WARNING - CODE IS NOT UPDATED 
/// NEEDED AN APPROPRIATE PARAMETER TUNING!!! 
/// WORKS BUT PARAMETERS ARE IN GENERAL OFF!!!
/// WARNING
...
arrayV2 GetParamWithSample(arrayD4 sample1, arrayD4 sample2); // Calculation of 3D line from two points
...
double ClusterTest(double &sumvalue, double &totalenergy, vector<int> &inliers); // function used to compare the clusters.
...
int Ransac_Analyze(string input_file = "")
{
    ...
    while (rdr.Next())
    {
        ...
        if (rdr.GetCurrentEntry() % 10000 == 0)
        {
        ...
        TVector3 cane = vertex;
        if (cane.x() != 0) // Saving only events with a vertex different from zero.
        {
            fOutTree->Fill();
        }
