...
int numx = 256;
int numy = 256;
double numz = 256; // numz depends on time and drift speed. (100: default value)
...
bool InOscar(int gid)
{
    ...
    if ((gid > 21000) && (gid < 21234) && (gid != 21011) &&
        (gid != 21022) && (gid != 21045) && (gid != 21056) &&
        (gid != 21111) && (gid != 21122) && (gid != 21145) &&
        (gid != 21156) && (gid != 21231) /*
        && ( gid != 21211 ) && ( gid != 21222 ) */
    )
    {
...
int Ransac(TString ifname = "")
{
    ...
    // cout << "Set event num:" ;
    // cin >> toanalyse;
    double threshold = 10000;    // minimum energy requested for a single cluster.
    ...
    double trsize = 30;         // min number of pads required in order to consider a cluster a real track
    ...
    while (rdr.Next())
    {
        ...
        if (event->getEventNumber() == 11 /* toanalyse*/)
        {
            ...
            */
            ...
            for (unsigned int i = 0; i < vectracks.size() && i < 6; i++)
            {
                ...
                { // now drawing line
                    ...
                    TVector3 base1 = vectracks[i].getBasepoint() - vectracks[i].getDirection() * 1000; //- vectracks[i].getDirection() * ((1 + vectracks[i].getBasepoint().x()) / vectracks[i].getDirection().x());
                    drawObj->SetNextPoint(base1.x(), base1.y(), base1.z());

                    TVector3 bn = vectracks[i].getBasepoint() + vectracks[i].getDirection() * 1000;//((66 - vectracks[i].getBasepoint().y()) / vectracks[i].getDirection().y());

                    drawObj->SetNextPoint(bn.x(), bn.y(), bn.z());
                    drawObj->SetLineColor(2);
                    drawObj->SetLineWidth(1);
                    drawObj->Draw("same");
                }
            }
...
arrayV2 MinuitForTrack(cFittedLine<int> &track)
{
    ...
    gMinuit->DefineParameter(1, "Ay", 64, 0.01, -100, 164);
    gMinuit->DefineParameter(2, "Az", 250, 0.01, -100, 500);
