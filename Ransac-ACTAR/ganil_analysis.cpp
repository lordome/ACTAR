...
int numx = 256;
int numy = 256;
double numz = 500; // numz depends on time and drift speed. (100: default value)
...
int fit(/*TString ifname=""*/)
{
    // Setting Hyperparameters
    gROOT->SetBatch(kFALSE);

    int toanalyse; //=1746 ;//Proviamo a semplificare per un primo momento. Questo ci dice il numero di evento da analizzare.
    // cout << "Set event num:"; // try 1746 for 410Cal.root input file.
    // cin >> toanalyse;
    double threshold = 20000;   // minimum energy requested for a single cluster.
    ...
    double fwidth = 2.5;        // maximum distance from model accepted in clustering for beam tracks
    double SqVertex_Width = 55; // maximum distance accepted between two different cluster
    double loops = 500;         // number of loops, i.e. number of random couples chosen.
    double trsize = 40;         // min number of pads required in order to consider a cluster a real track
    double besize = 150;        // min number of pads required in order to consider a cluster a real track FOR BEAM

    // end Setting Hyperparameters

    // Opening the input file.
    TFile *ifile = new TFile("outputPrecalibrator.root", "READ");
    if (ifile->IsZombie())
    {
        printf("Unable to open inputfile: %s", "outputPrecalibrator.root");
        return -1;
    }
    ...
    while (rdr.Next())
    {
        ...
        { // looking for normal tracks. The method used is exactly the previous one.
            ...
            {
                unfittedPoints[bestinliers[i]] = -1;
            } // setto i punti già utilizzati a -1.
...
Double_t GetError(arrayV2 model, arrayD4 p)
{
    ...
    TVector3 d = {p[0] - H[0], p[1] - H[1], (p[2] - H[2]) / 20}; // it was necessary to put this factor 10 due to measurement units (as I have set the precalibrator).
...
arrayV2 MinuitForTrack(cFittedLine<int> &track)
{
    ...
    Double_t maxY = numy;
    Double_t maxZ = numz;
    gMinuit->DefineParameter(0, "Ax", 0, 0.01, -100, 256);
    gMinuit->DefineParameter(1, "Ay", 32, 0.01, -100, 256);
    gMinuit->DefineParameter(2, "Az", 8, 0.01, -100, 1000);
