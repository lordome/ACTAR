...
// template <class T>
// cTrackerRansacSpeed<T>::cTrackerRansacSpeed(){};

// template <class T>
// cTrackerRansacSpeed<T>::~cTrackerRansacSpeed(){};

template <class T>
void cTrackerRansacSpeed<T>::setTracksEnergyThreshold(double v)
{
  tracksEnergyThreshold = v;
};
template <class T>
void cTrackerRansacSpeed<T>::setBeamEnergyThreshold(double v)
{
  beamEnergyThreshold = v;
};
template <class T>
void cTrackerRansacSpeed<T>::setTracksWidth(double v)
{
  tracksWidth = v;
};
template <class T>
void cTrackerRansacSpeed<T>::setBeamWidth(double v) { beamWidth = v; };
template <class T>
void cTrackerRansacSpeed<T>::setLoopsNumber(double v) { loopsNumber = v; };
template <class T>
void cTrackerRansacSpeed<T>::setTrackMinSize(double v) { trackMinSize = v; };
template <class T>
void cTrackerRansacSpeed<T>::setBeamMinSize(double v) { beamMinSize = v; };

template <class T>
void cTrackerRansacSpeed<T>::addPoint(const T &v)
{
  points.push_back(v);
}
// Calculation of 3D line from two vectors
template <class T>
std::array<TVector3, 2> cTrackerRansacSpeed<T>::GetParamWithSample(std::array<Double_t, 4> sample1, std::array<Double_t, 4> sample2)
{
...
template <class T>
void cTrackerRansacSpeed<T>::Ransac(double &minEnergy, double &minSize, double &widthTrack, bool &beamTracks, bool &oneSidedTracks, bool &isFittable)
{
  ...
  { // There are particular conditions, the points chosen randomly must have same y & z, the energy threshold is very high, the number of pads required has to be high.
    ...
    std::vector<double> iterations_i;
    std::vector<double> energies_i;
    while (iterations < loopsNumber)
    {
      ...
      iterations_i.push_back(bestInliers.size());
      energies_i.push_back(bestEnergy);
      ++iterations;
    }
    ...
    iterations_i.push_back(50000);
    iterSize.push_back(iterations_i);

    energies_i.push_back(50000);
    iterEnergy.push_back(iterations_i);
...
template <class T>
void cTrackerRansacSpeed<T>::fitLines()
{
  trackLines(lines);
}

template <class T>
Double_t cTrackerRansacSpeed<T>::GetError(std::array<TVector3, 2> model, std::array<Double_t, 4> p, double zScale)
{
...
template <class T>
Double_t cTrackerRansacSpeed<T>::GetError_noscale(std::array<TVector3, 2> model, std::array<Double_t, 4> p)
{
  TVector3 a = {p[0] - model[0][0], p[1] - model[0][1], p[2] - model[0][2]};
  TVector3 H = model[0] + a.Dot(model[1]) * model[1];
...
template <class T>
Double_t cTrackerRansacSpeed<T>::ClusterTest(double &sumvalue, double &totalenergy, vector<T> &inliers)
{
