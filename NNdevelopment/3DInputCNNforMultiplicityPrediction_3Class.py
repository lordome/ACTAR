...
n_data = 20
test_s = 0.15
X_train, X_test, y_train, y_test = train_test_split((np.fromfile("3DdataNN.bin").reshape(
    n_data, 128, 128, 3)), np.loadtxt("vertSize.txt", max_rows=n_data), test_size=test_s)
...
for itTrain in y_train:
    if itTrain > 2:
        yTrain3class.append(2)
    if itTrain == 2:
        yTrain3class.append(1)
    if itTrain < 2:
        yTrain3class.append(0)

for itTest in y_test:
    if itTest > 2:
        yTest3class.append(2)
    if itTest == 2:
        yTest3class.append(1)
    if itTest < 2:
        yTest3class.append(0)

numberCategories = max(np.concatenate((yTrain3class, yTest3class)))+1
y_train = keras.utils.to_categorical(yTrain3class)
y_test = keras.utils.to_categorical(yTest3class)
...
l_pol = [2, 2, 3]
l_dense = []
model = CNNDefinition(l_fil, l_ker, l_pol, l_dense,
                        (128, 128, 3), numberCategories, l_rate=0.01)
...
model.save(
    'model_3DSimple_LargeTest_InputShape128_128_3_Accuracy85Ransac_trainSize30000x85.h5')
