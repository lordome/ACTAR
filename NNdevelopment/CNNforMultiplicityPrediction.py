...
test_s = 0.15
setShuffle = False
X_train, X_test, y_train, y_test = train_test_split(((np.load("mockData1D.npy"))[:n_data*128*128]).reshape(n_data, 128, 128, 1), np.loadtxt("countVertexTracks.txt", max_rows=n_data),
                                                    shuffle=setShuffle, test_size=test_s)

numberCategories = max(np.concatenate((y_train, y_test)))+1

y_train = keras.utils.to_categorical(y_train, num_classes=int(numberCategories))
y_test = keras.utils.to_categorical(y_test, num_classes=int(numberCategories))
...
l_pol = [2,2, 2]
l_dense = []
model = CNNDefinition(l_fil, l_ker, l_pol, l_dense, (128,128,1), N_categ = numberCategories, l_rate = 0.001)

#Fit network accordingly to data loaded in previous step
history = Fit(model, X_train, y_train, X_test, y_test, batch = 128, epochs = 80)
...
model.save('model_1DSimple_LargeTest_InputShape128_128_1_Accuracy85Ransac_trainSize30000x85.h5')
