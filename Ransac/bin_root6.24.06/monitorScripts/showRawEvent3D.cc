...
void showRawEvent3D(TString fname, Int_t eventNumber) {
  TH3 *pad = new TH3F(Form("pad_%d", eventNumber), "Pad hit projection", 64, -0.5, 63.5, 32, -0.5, 31.5, 512, -0.5, 511.5);
  ...
  for (auto &hit: event->getHits()) {
    ...
    pad->Fill(hitInfo.getCol(), hitInfo.getRow(), hit.getPeakTime(), hit.getPeakHeight());
  }
